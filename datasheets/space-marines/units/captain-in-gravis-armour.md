---
slug: captain-in-gravis-armour
faction: space-marines
sub_factions: [adeptus-astartes]
epic_hero: false
battleline: false
is_character: true
loadouts:
  - models: 1
    points: 80
    default: true
keywords:
  faction: [Adeptus Astartes, Imperium]
  unit: [Infantry, Character, Grenades, Gravis, Captain]
led_by: []
grants_to_attached_unit:
  description: "While leading a unit, this model's Rites of Battle ability lets one unit from your army with this ability use it once per battle round when targeted with a Stratagem, reducing the CP cost of that Stratagem by 1CP (to a minimum of 0CP). Refuse to Yield: each time an attack is allocated to this model, halve the Damage characteristic of that attack (rounding up)."
  unmodelled: "Rites of Battle is an army-wide CP discount, not a per-attached-unit combat modifier; Refuse to Yield is a self-only damage-halving defensive buff. Engine has no CP/Stratagem economy nor allocated-attack damage modifiers."
---

# Captain in Gravis Armour

**Faction:** Space Marines (Adeptus Astartes)
**Source:** Codex: Space Marines (10th Edition) / Wahapedia

---

## Keywords

**Faction Keywords:** Adeptus Astartes, Imperium
**Unit Keywords:** Infantry, Character, Grenades, Imperium, Gravis, Captain

---

## Base

- **Shape:** round
- **Diameter:** 40mm
- **Flight stem:** no

---

## Profile

| M  | T | Sv | W | Ld | OC |
|----|---|----|---|----|----|
| 5" | 6 | 3+ | 6 | 6+ | 1  |

**Invulnerable Save:** 4+

---

## Ranged Weapons

| Weapon                            | Range | A | BS | S | AP | D | Keywords                    |
|-----------------------------------|-------|---|----|---|----|---|-----------------------------|
| Boltstorm gauntlet (pistol)       | 12"   | 3 | 2+ | 4 | -1 | 1 | [PISTOL]                    |
| Master-crafted heavy bolt rifle   | 30"   | 2 | 2+ | 5 | -1 | 3 | [ASSAULT], [HEAVY]          |

---

## Melee Weapons

| Weapon                      | Range | A | WS | S | AP | D | Keywords           |
|-----------------------------|-------|---|----|---|----|---|--------------------|
| Master-crafted power weapon | Melee | 6 | 2+ | 5 | -2 | 2 | —                  |
| Power fist                  | Melee | 5 | 2+ | 8 | -2 | 2 | —                  |
| Relic blade                 | Melee | 2 | 2+ | 5 | -2 | 2 | [EXTRA ATTACKS]    |
| Relic chainsword            | Melee | 3 | 2+ | 4 | -1 | 2 | [EXTRA ATTACKS]    |
| Relic fist                  | Melee | 1 | 2+ | 8 | -2 | 2 | [EXTRA ATTACKS]    |

---

## Abilities

### Core
Leader

### Faction
**Oath of Moment.** If your Army Faction is Adeptus Astartes, at the start of your Command phase, select one unit from your opponent's army. Until the start of your next Command phase, that enemy unit is your Oath of Moment target. Each time a model with this ability makes an attack that targets your Oath of Moment target, you can re-roll the Hit roll, and if you are using a Codex: Space Marines Detachment and your army does not include one or more units with the Black Templars, Blood Angels, Dark Angels, Deathwatch or Space Wolves keywords, you can also add 1 to the Wound roll.

### Rites of Battle
Once per battle round, one unit from your army with this ability can use it when targeted with a Stratagem. If it does, reduce the CP cost of that Stratagem by 1CP, to a minimum of 0CP.

### Refuse to Yield
Each time an attack is allocated to this model, halve the Damage characteristic of that attack (rounding up).

---

## Unit Composition

- 1 Captain in Gravis Armour

This model is equipped with: master-crafted heavy bolt rifle; master-crafted power weapon.

---

## Wargear Options

- This model's master-crafted heavy bolt rifle and master-crafted power weapon can be replaced with one of the following configurations:
  - 1 boltstorm gauntlet, 1 power fist, and 1 relic chainsword
  - 1 boltstorm gauntlet, 1 power fist, and 1 relic blade
  - 1 boltstorm gauntlet, 1 power fist, and 1 relic fist

---

## Leader

This model can be attached to the following units (all GRAVIS):

- Aggressor Squad
- Eradicator Squad
- Heavy Intercessor Squad

> **CANNOT lead Tacticus units** (Intercessor, Infernus, Hellblaster, Sternguard, Bladeguard, Tactical, Assault Intercessor). Tacticus leaders are the standard Captain.
> **CANNOT lead Terminator, Inceptor, Phobos units.**
> Indomitor Kill Team also appears on Wahapedia's list but is a Deathwatch unit — not legal in pure Ultramarines.

---

## Points

**80 points** (1 model)

---

## Source

- Wahapedia: https://wahapedia.ru/wh40k10ed/factions/space-marines/Captain-In-Gravis-Armour
- Munitorum Field Manual v3.9 (Feb 2026), Codex: Space Marines section

<!-- Last verified: 2026-04-21 — source: Wahapedia https://wahapedia.ru/wh40k10ed/factions/space-marines/Captain-In-Gravis-Armour / MFM v3.9 -->
