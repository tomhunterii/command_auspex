---
slug: lieutenant-in-phobos-armour
faction: space-marines
sub_factions: [adeptus-astartes]
epic_hero: false
battleline: false
is_character: true
loadouts:
  - models: 1
    points: 55
    default: true
keywords:
  faction: [Adeptus Astartes, Imperium]
  unit: [Infantry, Character, Grenades, Phobos, Lieutenant]
led_by: []
grants_to_attached_unit:
  weapon_abilities:
    lethal_hits: true
  description: "Tactical Precision: while leading a unit, weapons equipped by models in that unit have the [LETHAL HITS] ability. Strategic Dispersal (self): in your Shooting phase, after this model's unit has shot, if it is not within Engagement Range of one or more enemy units, it can make a Normal move of up to 6\"; if it does, until the end of the turn, that unit is not eligible to declare a charge."
  unmodelled: "Strategic Dispersal is a post-shooting reposition + charge restriction the engine does not model — encoded in description only. LETHAL HITS is captured via weapon_abilities."
---

# Lieutenant in Phobos Armour

**Faction:** Space Marines (Adeptus Astartes)
**Source:** Codex: Space Marines (10th Edition) / Wahapedia / MFM v3.9

---

## Keywords

**Faction Keywords:** Adeptus Astartes, Imperium
**Unit Keywords:** Infantry, Character, Grenades, Imperium, Phobos, Lieutenant

---

## Base

- **Shape:** round
- **Diameter:** 40mm
- **Flight stem:** no

---

## Profile

| M  | T | Sv | W | Ld | OC |
|----|---|----|---|----|----|
| 6" | 4 | 3+ | 4 | 6+ | 1  |

---

## Ranged Weapons

| Weapon                              | Range | A | BS | S | AP | D | Keywords |
|-------------------------------------|-------|---|----|---|----|---|----------|
| Bolt pistol                         | 12"   | 1 | 2+ | 4 | 0  | 1 | [PISTOL] |
| Master-crafted scoped bolt carbine  | 24"   | 2 | 2+ | 4 | 0  | 2 | —        |

---

## Melee Weapons

| Weapon                | Range | A | WS | S | AP | D | Keywords           |
|-----------------------|-------|---|----|---|----|---|--------------------|
| Paired combat blades  | Melee | 5 | 2+ | 4 | -1 | 1 | [SUSTAINED HITS 1] |

---

## Abilities

### Core
Deep Strike, Infiltrators, Leader, Scouts 6"

### Faction
**Oath of Moment.** If your Army Faction is Adeptus Astartes, at the start of your Command phase, select one unit from your opponent's army. Until the start of your next Command phase, that enemy unit is your Oath of Moment target. Each time a model with this ability makes an attack that targets your Oath of Moment target, you can re-roll the Hit roll, and if you are using a Codex: Space Marines Detachment and your army does not include one or more units with the Black Templars, Blood Angels, Dark Angels, Deathwatch or Space Wolves keywords, you can also add 1 to the Wound roll.

### Tactical Precision
While this model is leading a unit, weapons equipped by models in that unit have the [LETHAL HITS] ability.

### Strategic Dispersal
In your Shooting phase, after this model's unit has shot, if it is not within Engagement Range of one or more enemy units, it can make a Normal move of up to 6". If it does, until the end of the turn, that unit is not eligible to declare a charge.

---

## Unit Composition

- 1 Lieutenant in Phobos Armour

This model is equipped with: bolt pistol; master-crafted scoped bolt carbine; paired combat blades.

---

## Wargear Options

None.

---

## Leader

This model can be attached to the following units:

- Incursor Squad
- Infiltrator Squad
- Reiver Squad

> **PHOBOS-only leader.** Cannot lead Tacticus, Gravis, Terminator, or Inceptor units. Note: cannot lead Eliminator Squad or Scout Squad (unlike the Captain in Phobos Armour).

---

## Points

**55 points** (1 model)

---

## Source

- Wahapedia: https://wahapedia.ru/wh40k10ed/factions/space-marines/Lieutenant-In-Phobos-Armour
- Munitorum Field Manual v3.9 (Feb 2026), Codex: Space Marines section

<!-- Last verified: 2026-04-28 — source: Wahapedia https://wahapedia.ru/wh40k10ed/factions/space-marines/Lieutenant-In-Phobos-Armour / MFM v3.9 -->
