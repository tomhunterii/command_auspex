# Intercessor Squad

**Faction:** Space Marines (Adeptus Astartes)
**Source:** Codex: Space Marines (10th Edition) / Wahapedia

---

## Keywords

**Faction Keywords:** Adeptus Astartes
**Unit Keywords:** Infantry, Battleline, Grenades, Imperium, Tacticus, Intercessor Squad

---

## Base

- **Shape:** round
- **Diameter:** 32mm
- **Flight stem:** no

---

## Profile

| M  | T | Sv | W | Ld | OC |
|----|---|----|---|----|----|
| 6" | 4 | 3+ | 2 | 6+ | 2  |

---

## Ranged Weapons

| Weapon                                | Range | A  | BS | S | AP | D | Keywords              |
|---------------------------------------|-------|----|----|---|----|---|-----------------------|
| Bolt rifle                            | 24"   | 2  | 3+ | 4 | -1 | 1 | [ASSAULT], [HEAVY]    |
| Bolt pistol                           | 12"   | 1  | 3+ | 4 | 0  | 1 | [PISTOL]              |
| Astartes grenade launcher – frag      | 24"   | D3 | 3+ | 4 | 0  | 1 | [BLAST]               |
| Astartes grenade launcher – krak      | 24"   | 1  | 3+ | 9 | -2 | 3 | —                     |
| Hand flamer                           | 12"   | D6 | N/A| 3 | 0  | 1 | [IGNORES COVER], [PISTOL], [TORRENT] |
| Plasma pistol – standard              | 12"   | 1  | 3+ | 7 | -2 | 1 | [PISTOL]              |
| Plasma pistol – supercharge           | 12"   | 1  | 3+ | 8 | -3 | 2 | [HAZARDOUS], [PISTOL] |

---

## Melee Weapons

| Weapon               | Range | A | WS | S | AP | D | Keywords              |
|----------------------|-------|---|----|---|----|---|-----------------------|
| Close combat weapon  | Melee | 3 | 3+ | 4 | 0  | 1 | —                     |
| Astartes chainsword  | Melee | 5 | 3+ | 4 | -1 | 1 | —                     |
| Power weapon         | Melee | 4 | 3+ | 5 | -2 | 1 | —                     |
| Power fist           | Melee | 3 | 3+ | 8 | -2 | 2 | —                     |
| Thunder hammer       | Melee | 3 | 4+ | 8 | -2 | 2 | [DEVASTATING WOUNDS]  |

---

## Abilities

### Faction
**Oath of Moment.** If your Army Faction is Adeptus Astartes, at the start of your Command phase, select one unit from your opponent's army. Until the start of your next Command phase, that enemy unit is your Oath of Moment target. Each time a model with this ability makes an attack that targets your Oath of Moment target, you can re-roll the Hit roll, and if you are using a Codex: Space Marines Detachment and your army does not include one or more units with the Black Templars, Blood Angels, Dark Angels, Deathwatch or Space Wolves keywords, you can also add 1 to the Wound roll.

### Target Elimination
Each time this unit is selected to shoot, it can use this ability. If it does, until the end of the phase, add 2 to the Attacks characteristic of bolt rifles equipped by models in this unit and you can only select one enemy unit as the target of all of this unit's attacks.

---

## Unit Composition

- 1 Intercessor Sergeant
- 4–9 Intercessors

Every model is equipped with: bolt pistol; bolt rifle; close combat weapon.

---

## Wargear Options

- The Intercessor Sergeant's bolt rifle can be replaced with one of the following: 1 Astartes chainsword; 1 hand flamer; 1 plasma pistol; 1 power weapon.
- The Intercessor Sergeant's close combat weapon can be replaced with one of the following: 1 Astartes chainsword; 1 power fist; 1 power weapon; 1 thunder hammer.
- For every 5 models in this unit, 1 Intercessor's bolt rifle can be replaced with 1 Astartes grenade launcher.

---

## Led By

This unit can be led by the following Characters (mono-Codex SM / Ultramarines):

- Adrax Agatone
- Ancient
- Apothecary (standard Tacticus — NOT Apothecary Biologis, who is Gravis-only)
- Captain
- Captain Titus (Demetrian Titus)
- Chaplain
- Chief Librarian Tigurius
- Judiciar
- Kor'sarro Khan
- Librarian
- Lieutenant
- Marneus Calgar in Armour of Antilochus
- Techmarine
- Uriel Ventris

> Wahapedia list also includes faction-specific leaders (Black Templars Castellan/Marshal/Grimaldus/Emperor's Champion, Blood Angels, Dark Angels, Space Wolves, Deathwatch variants). Those are NOT legal in pure Ultramarines / Blade of Ultramar lists.

---

## Points

| Models | Points |
|--------|--------|
| 5      | 80     |
| 10     | 160    |

---

## Source

- Wahapedia: https://wahapedia.ru/wh40k10ed/factions/space-marines/Intercessor-Squad
- Munitorum Field Manual v3.9 (Feb 2026), Codex: Space Marines section

<!-- Last verified: 2026-04-21 — source: Wahapedia https://wahapedia.ru/wh40k10ed/factions/space-marines/Intercessor-Squad / MFM v3.9 -->
