# Captain in Terminator Armour

**Faction:** Space Marines (Adeptus Astartes)
**Source:** Codex: Space Marines (10th Edition) / Wahapedia

---

## Keywords

**Faction Keywords:** Adeptus Astartes
**Unit Keywords:** Infantry, Character, Imperium, Terminator, Captain

---

## Base

- **Shape:** round
- **Diameter:** 50mm
- **Flight stem:** no

---

## Profile

| M  | T | Sv | W | Ld | OC |
|----|---|----|---|----|----|
| 5" | 5 | 2+ | 6 | 6+ | 1  |

**Invulnerable Save:** 4+

---

## Ranged Weapons

| Weapon        | Range | A | BS | S | AP | D | Keywords                                          |
|---------------|-------|---|----|---|----|---|---------------------------------------------------|
| Storm bolter  | 24"   | 2 | 2+ | 4 | 0  | 1 | [RAPID FIRE 2]                                    |
| Combi-weapon  | 24"   | 1 | 3+ | 4 | 0  | 1 | [ANTI-INFANTRY 4+], [DEVASTATING WOUNDS], [RAPID FIRE 1] |

---

## Melee Weapons

| Weapon        | Range | A | WS | S | AP | D | Keywords |
|---------------|-------|---|----|---|----|---|----------|
| Relic fist    | Melee | 5 | 2+ | 8 | -2 | 2 | —        |
| Relic weapon  | Melee | 6 | 2+ | 5 | -2 | 2 | —        |

---

## Abilities

### Core
Deep Strike, Leader

### Faction
**Oath of Moment.** If your Army Faction is Adeptus Astartes, at the start of your Command phase, select one unit from your opponent's army. Until the start of your next Command phase, that enemy unit is your Oath of Moment target. Each time a model with this ability makes an attack that targets your Oath of Moment target, you can re-roll the Hit roll, and if you are using a Codex: Space Marines Detachment and your army does not include one or more units with the Black Templars, Blood Angels, Dark Angels, Deathwatch or Space Wolves keywords, you can also add 1 to the Wound roll.

### Rites of Battle
Once per battle round, one unit from your army with this ability can use it when targeted with a Stratagem. If it does, reduce the CP cost of that Stratagem by 1CP, to a minimum of 0CP.

### The Imperium's Sword
You can re-roll Charge rolls made for this model's unit.

---

## Unit Composition

- 1 Captain in Terminator Armour

This model is equipped with: storm bolter; relic weapon.

---

## Wargear Options

- This model's storm bolter can be replaced with 1 combi-weapon.
- This model's relic weapon can be replaced with 1 relic fist.

---

## Leader

This model can be attached to the following units:

- Terminator Assault Squad
- Terminator Squad

> **CANNOT lead Sternguard Veteran Squad.** Confirmed against Wahapedia: Sternguard's LED BY list does not include Captain in Terminator Armour, and this Captain's LEADER list is TERMINATOR-only.
> **CANNOT lead Bladeguard, Hellblasters, Intercessors, or any Tacticus unit.** Terminator-only.
> Wahapedia also lists Deathwing / Deathwatch Terminator units — not legal in pure Ultramarines.

---

## Points

**95 points** (1 model)

---

## Source

- Wahapedia: https://wahapedia.ru/wh40k10ed/factions/space-marines/Captain-In-Terminator-Armour
- Munitorum Field Manual v3.9 (Feb 2026), Codex: Space Marines section

<!-- Last verified: 2026-04-21 — source: Wahapedia https://wahapedia.ru/wh40k10ed/factions/space-marines/Captain-In-Terminator-Armour / MFM v3.9 -->
