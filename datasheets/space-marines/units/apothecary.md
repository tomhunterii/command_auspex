# Apothecary

**Faction:** Space Marines (Adeptus Astartes)
**Source:** Codex: Space Marines (10th Edition) / Wahapedia

> Standard Tacticus-armour Apothecary. For the Gravis-armour offensive buff variant, see `apothecary-biologis.md`.

---

## Keywords

**Faction Keywords:** Adeptus Astartes
**Unit Keywords:** Infantry, Character, Grenades, Imperium, Tacticus, Apothecary

---

## Base

- **Shape:** round
- **Diameter:** 40mm
- **Flight stem:** no

---

## Profile

| M  | T | Sv | W | Ld | OC |
|----|---|----|---|----|----|
| 6" | 4 | 3+ | 4 | 6+ | 1  |

---

## Ranged Weapons

| Weapon                | Range | A | BS | S | AP | D | Keywords |
|-----------------------|-------|---|----|---|----|---|----------|
| Absolvor bolt pistol  | 18"   | 1 | 3+ | 5 | -1 | 2 | [PISTOL] |
| Reductor pistol       | 3"    | 1 | 3+ | 4 | -4 | 2 | [PISTOL] |

---

## Melee Weapons

| Weapon              | Range | A | WS | S | AP | D | Keywords |
|---------------------|-------|---|----|---|----|---|----------|
| Close combat weapon | Melee | 4 | 3+ | 4 | 0  | 1 | —        |

---

## Abilities

### Core
Leader

### Faction
**Oath of Moment.** If your Army Faction is Adeptus Astartes, at the start of your Command phase, select one unit from your opponent's army. Until the start of your next Command phase, that enemy unit is your Oath of Moment target. Each time a model with this ability makes an attack that targets your Oath of Moment target, you can re-roll the Hit roll, and if you are using a Codex: Space Marines Detachment and your army does not include one or more units with the Black Templars, Blood Angels, Dark Angels, Deathwatch or Space Wolves keywords, you can also add 1 to the Wound roll.

### Narthecium
While this model is leading a unit, in your Command phase, you can return 1 destroyed model (excluding CHARACTER models) to that unit.

### Gene-seed Recovery
When this model's Bodyguard unit is destroyed, roll one D6: on a 2+, you gain 1CP.

---

## Unit Composition

- 1 Apothecary

This model is equipped with: absolvor bolt pistol; reductor pistol; close combat weapon.

---

## Wargear Options

None.

---

## Leader

This model can be attached to the following units (all TACTICUS):

- Assault Intercessor Squad
- Desolation Squad
- Devastator Squad
- Hellblaster Squad
- Infernus Squad
- Intercessor Squad
- Sternguard Veteran Squad
- Tactical Squad

> **CANNOT lead Bladeguard Veteran Squad** (notably absent).
> **CANNOT lead Gravis, Terminator, Inceptor, or Phobos units.** For Gravis buffing, use the Apothecary Biologis (a different datasheet — but note Biologis grants Lethal Hits aura, NOT Feel No Pain / Narthecium revive).

---

## Points

**50 points** (1 model)

---

## Source

- Wahapedia: https://wahapedia.ru/wh40k10ed/factions/space-marines/Apothecary
- Munitorum Field Manual v3.9 (Feb 2026), Codex: Space Marines section

<!-- Last verified: 2026-04-21 — source: Wahapedia https://wahapedia.ru/wh40k10ed/factions/space-marines/Apothecary / MFM v3.9. Key distinction from Biologis: Apothecary (Tacticus) grants Narthecium model-return + potential CP on Bodyguard death; Biologis grants Lethal Hits aura and scales OC on kills. Apothecary does NOT grant Feel No Pain either — that remains a common misconception. -->
