---
slug: desolation-squad
faction: space-marines
sub_factions: [adeptus-astartes]
epic_hero: false
battleline: false
is_character: false
loadouts:
  - models: 5
    points: 200
    default: true
keywords:
  faction: [Adeptus Astartes, Imperium]
  unit: [Infantry, Grenades, Tacticus, Desolation Squad]
led_by:
  - apothecary
  - chief-librarian-tigurius
---

# Desolation Squad

**Faction:** Space Marines (Adeptus Astartes)
**Source:** Codex: Space Marines (10th Edition) / Wahapedia / MFM v3.9

---

## Keywords

**Faction Keywords:** Adeptus Astartes, Imperium
**Unit Keywords:** Infantry, Grenades, Imperium, Tacticus, Desolation Squad

---

## Base

- **Shape:** round
- **Diameter:** 32mm
- **Flight stem:** no

---

## Profile

| M  | T | Sv | W | Ld | OC |
|----|---|----|---|----|----|
| 6" | 4 | 3+ | 2 | 6+ | 1  |

---

## Ranged Weapons

| Weapon                    | Range | A    | BS | S  | AP | D     | Keywords                  |
|---------------------------|-------|------|----|----|----|-------|---------------------------|
| Bolt pistol               | 12"   | 1    | 3+ | 4  | 0  | 1     | [PISTOL]                  |
| Castellan launcher        | 36"   | D3   | 3+ | 4  | 0  | 1     | [BLAST], [INDIRECT FIRE]  |
| Superfrag rocket launcher | 48"   | D6+1 | 4+ | 5  | 0  | 1     | [BLAST], [HEAVY]          |
| Superkrak rocket launcher | 48"   | 1    | 4+ | 10 | -2 | D6+1  | [HEAVY]                   |
| Vengor launcher           | 48"   | D6   | 2+ | 7  | -1 | 2     | [BLAST], [INDIRECT FIRE]  |

> **INDIRECT FIRE handling (per Captain's standing rules):** Indirect Fire applies -1 to hit only. It does NOT grant automatic cover to the target — cover remains positional, only models physically in cover gain it.

---

## Melee Weapons

| Weapon              | Range | A | WS | S | AP | D | Keywords |
|---------------------|-------|---|----|---|----|---|----------|
| Close combat weapon | Melee | 3 | 3+ | 4 | 0  | 1 | —        |

---

## Abilities

### Faction
**Oath of Moment.** If your Army Faction is Adeptus Astartes, at the start of your Command phase, select one unit from your opponent's army. Until the start of your next Command phase, that enemy unit is your Oath of Moment target. Each time a model with this ability makes an attack that targets your Oath of Moment target, you can re-roll the Hit roll, and if you are using a Codex: Space Marines Detachment and your army does not include one or more units with the Black Templars, Blood Angels, Dark Angels, Deathwatch or Space Wolves keywords, you can also add 1 to the Wound roll.

### Targeter Optics
Each time this unit Remains Stationary, until the start of your next Movement phase, ranged weapons equipped by models in this unit have the [IGNORES COVER] ability.

---

## Unit Composition

- 1 Desolation Sergeant
- 4 Desolation Marines

Every model is equipped with: bolt pistol; superfrag rocket launcher; close combat weapon.

---

## Wargear Options

- Any model's superfrag rocket launcher can be replaced with 1 superkrak rocket launcher.
- The Desolation Sergeant's superfrag rocket launcher (or superkrak rocket launcher) can be replaced with 1 castellan launcher.
- The Desolation Sergeant's superfrag rocket launcher (or superkrak rocket launcher) can alternatively be replaced with 1 vengor launcher (per Wahapedia wargear options).

---

## Led By

This unit can be led by the following Characters (mono-Codex SM / Ultramarines):

- Apothecary
- Chief Librarian Tigurius

> Wahapedia's full Led By list also includes generic Ancient, generic Librarian, Techmarine, and (cross-Chapter / ally) Sanguinary Priest, Brother Corbulo. Those datasheets do not exist in this repo, so they are absent from `led_by:`. Rules-legality vs. flavour-incongruity distinction: all are rules-legal in Codex SM Detachments; Sanguinary Priest / Brother Corbulo are flavour-incongruous in mono-Ultramarines.
> **Conflict flag (per MFM-skepticism directive):** Captain Hunter's brief asserted **Iron Father Feirros** and **Tor Garadon** as the Desolation Squad's leaders. **Wahapedia 2026-04-29 disagrees** — neither is listed in this unit's Led By. Iron Father Feirros and Tor Garadon are Wahapedia-confirmed leaders of the Heavy Intercessor Squad, not Desolation. Adopting Wahapedia for Desolation. Captain to confirm if a different/older list applied.
> Not an Epic Hero — the squad CAN host an Enhancement on its leader (subject to that leader's own eligibility).

---

## Points

**200 points** (5 models)

---

## Source

- Wahapedia: https://wahapedia.ru/wh40k10ed/factions/space-marines/Desolation-Squad
- Munitorum Field Manual v3.9 (Feb 2026), Codex: Space Marines section
- Captain's brief (2026-04-29) listed Iron Father Feirros + Tor Garadon as leaders; Wahapedia does NOT list them. Adopted Wahapedia, flagged conflict.

<!-- Last verified: 2026-04-29 — source: Wahapedia https://wahapedia.ru/wh40k10ed/factions/space-marines/Desolation-Squad / MFM v3.9. Conflict flagged on Iron Father / Tor Garadon attachments. -->
