# Carnifexes

**Faction:** Tyranids
**Source:** Codex: Tyranids (10th Edition) / Wahapedia

> 1–2 Carnifex Monsters per unit. Dedicated shock-attack platform; loadout dictates whether the brick is Anti-Infantry shooting, Anti-Vehicle shooting, or a pure melee battering ram.

---

## Keywords

**Faction Keywords:** Tyranids
**Unit Keywords:** Monster, Great Devourer, Carnifexes

---

## Base

- **Shape:** oval
- **Dimensions:** 105mm × 70mm  (length × width)
- **Flight stem:** no

---

## Profile

| M  | T | Sv | W | Ld | OC |
|----|---|----|---|----|----|
| 8" | 9 | 2+ | 8 | 8+ | 3  |

**Invulnerable Save:** None

> Profile is per-model. A full 2-Carnifex unit fields 16 total wounds across two T9 2+ bodies.

---

## Ranged Weapons

| Weapon                              | Range | A    | BS | S | AP | D | Keywords  |
|-------------------------------------|-------|------|----|---|----|---|-----------|
| Bio-plasma                          | 12"   | D3   | 4+ | 7 | -2 | 1 | [BLAST]   |
| Deathspitters with slimer maggots   | 24"   | 6    | 4+ | 7 | -2 | 1 | —         |
| Devourers with brainleech worms     | 18"   | 12   | 4+ | 6 | 0  | 1 | —         |
| Heavy venom cannon                  | 36"   | D3   | 4+ | 9 | -2 | 3 | [BLAST]   |
| Spine banks                         | 6"    | 5    | 4+ | 5 | 0  | 1 | [ASSAULT] |
| Stranglethorn cannon                | 36"   | D6+1 | 4+ | 7 | -1 | 2 | [BLAST]   |

---

## Melee Weapons

| Weapon                                      | Range | A | WS | S  | AP | D    | Keywords         |
|---------------------------------------------|-------|---|----|----|----|------|------------------|
| Carnifex crushing claws                     | Melee | 4 | 4+ | 12 | -3 | D6+1 | —                |
| Carnifex extra scything talons              | Melee | 2 | 4+ | 9  | -2 | 3    | [EXTRA ATTACKS]  |
| Carnifex scything talons                    | Melee | 6 | 4+ | 9  | -2 | 3    | —                |
| Chitinous claws and teeth                   | Melee | 4 | 4+ | 6  | 0  | 1    | —                |

> The "extra scything talons" attack is resolved in addition to the model's other melee weapons per the [EXTRA ATTACKS] keyword.

---

## Abilities

### Core
**Deadly Demise 1.** When this model is destroyed, roll one D6 before removing it from play. On a 6, each unit within 6" of that model suffers 1 mortal wound.

### Faction
**Synapse.** While a Tyranids unit from your army is within Synapse Range of your army:
- Each time that unit takes a Battle-shock test, take that test on 3D6 instead of 2D6.
- Each time a model in that unit makes a melee attack, add 1 to the Strength characteristic of that attack.

**Shadow in the Warp.** Army-level rule (not printed on this datasheet); applies per the Codex: Tyranids army rules.

### Blistering Assault
*(Datasheet ability — note: the Captain's brief referred to this as "Living Battering Ram." The current 10th Edition / Wahapedia ability name is "Blistering Assault." "Living Battering Ram" was an older-edition name and does not appear on the current datasheet.)*

Each time an enemy unit is selected to shoot, after that unit has shot, if any models from this unit lost one or more wounds as a result of those attacks, this unit can make a Blistering Assault move. If it does, roll one D6, adding 2 to the result: each model in this unit can be moved a distance in inches up to the result, but this unit must finish that move as close as possible to the closest enemy unit. When doing so, those models can be moved within Engagement Range of that enemy unit. Each unit can only make one Blistering Assault move per phase.

---

## Unit Composition

- 1–2 Carnifexes

Every model is equipped with: Carnifex scything talons; Carnifex extra scything talons; chitinous claws and teeth.

---

## Wargear Options

- Any number of models can each have their **Carnifex extra scything talons** replaced with one of the following:
  - 1 deathspitters with slimer maggots
  - 1 devourers with brainleech worms
  - 1 heavy venom cannon
  - 1 stranglethorn cannon
  - 1 Carnifex crushing claws
- Any number of models can each have their **Carnifex scything talons** replaced with one of the following:
  - 1 deathspitters with slimer maggots
  - 1 devourers with brainleech worms
  - 1 Carnifex crushing claws
- Any number of models can each be equipped with one of the following (additive, no replacement):
  - 1 bio-plasma
  - 1 spine banks

> **Loadout math — quick guide:**
> - **Anti-Infantry shooting:** 2× devourers with brainleech worms (24 shots at S6) + bio-plasma + spine banks.
> - **Anti-Vehicle shooting:** 1× heavy venom cannon + 1× stranglethorn cannon + bio-plasma.
> - **Pure melee ram:** Keep both sets of scything talons (8 S9 AP-2 D3 attacks per Fex) OR swap one set to crushing claws (6 talon attacks + 4 S12 AP-3 D6+1 attacks) for target hardening.

---

## Led By

This unit can be led by the following Characters:

- **Old One Eye** (Epic Hero — cannot take Enhancements per 10th Ed rules).

---

## Points

| Models | Points |
|--------|--------|
| 1      | 90     |
| 2      | 180    |

> Unit cap is 2 Carnifexes per unit; for a third Carnifex, field a second unit.

---

## Source

- Wahapedia: https://wahapedia.ru/wh40k10ed/factions/tyranids/Carnifexes
- Munitorum Field Manual v3.9 (Feb 2026), Codex: Tyranids section

**Points conflict note:** Wahapedia lists 90 pts per model (180 for 2). No live conflict detected with MFM v3.9 at time of pull. Cross-verify against MFM PDF if divergence is suspected.

<!-- Last verified: 2026-04-23 — source: Wahapedia https://wahapedia.ru/wh40k10ed/factions/tyranids/Carnifexes / MFM v3.9 -->
