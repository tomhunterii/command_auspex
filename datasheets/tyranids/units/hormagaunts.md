---
slug: hormagaunts
faction: tyranids
sub_factions: [tyranids]
epic_hero: false
battleline: true
is_character: false
loadouts:
  - models: 10
    points: 65
    default: true
  - models: 20
    points: 130
keywords:
  faction: [Tyranids]
  unit: [Infantry, Battleline, Great Devourer, Endless Multitude, Hormagaunts]
led_by: []
---

# Hormagaunts

**Faction:** Tyranids
**Source:** Codex: Tyranids (10th Edition) / Wahapedia / MFM v3.9

> Cheap Battleline melee swarm — 10" move, [BOUNDING LEAP] (charge after Advance), and Endless Multitude returns models while a Synapse anchor is alive. Pure melee — no ranged weapons.

---

## Keywords

**Faction Keywords:** Tyranids
**Unit Keywords:** Infantry, Battleline, Great Devourer, Endless Multitude, Hormagaunts

---

## Base

- **Shape:** round
- **Diameter:** 28.5mm
- **Flight stem:** no

---

## Profile

| M   | T | Sv | W | Ld | OC |
|-----|---|----|---|----|----|
| 10" | 3 | 5+ | 1 | 8+ | 2  |

**Invulnerable Save:** —

---

## Ranged Weapons

None. Hormagaunts have no ranged weapons.

---

## Melee Weapons

| Weapon            | Range | A | WS | S | AP | D | Keywords |
|-------------------|-------|---|----|---|----|---|----------|
| Hormagaunt talons | Melee | 3 | 4+ | 3 | -1 | 1 | —        |

---

## Abilities

### Core
**Bounding Leap.** This unit is eligible to declare a charge in a turn in which it Advanced.

### Faction
**Synapse.** While a Tyranids unit from your army is within Synapse Range of your army: each time that unit takes a Battle-shock test, take that test on 3D6 instead of 2D6; and each time a model in that unit makes a melee attack, add 1 to the Strength characteristic of that attack.

**Shadow in the Warp.** Once per battle, in either player's Command phase, if one or more units from your army with this ability are on the battlefield, you can unleash the Shadow in the Warp. When you do, each enemy unit on the battlefield must take a Battle-shock test. Each time an enemy unit takes such a test, if it is within 6" of one or more Synapse units from your army, subtract 1 from that test.

### Endless Multitude
While this unit is within Synapse range of your army, each time one or more models in this unit are destroyed, roll one D6: on a 4+, one destroyed model is returned to this unit with its full wounds remaining. (Wording per Wahapedia datasheet; verify exact text in the codex before a tournament.)

---

## Unit Composition

- 10–20 Hormagaunts

Every model is equipped with: Hormagaunt talons.

---

## Wargear Options

None. Hormagaunts have no wargear options.

---

## Led By

This unit can be led by the following Characters (per Wahapedia):

- Tyranid Prime with Lash Whip

---

## Points

| Models | Points |
|--------|--------|
| 10     | 65     |
| 20     | 130    |

---

## Tactical Notes

- **10" move + Bounding Leap** = an Advance + charge threat bubble of (10 + D6 advance + 2D6 charge). Treat them as a 10–18" first-turn pressure piece.
- **Endless Multitude** requires Synapse range to function; keep a Hive Tyrant, Tyranid Warriors, Tyranid Prime, Tervigon or Neurothrope within 6" of the brood.
- **No invul, T3, 1W, 5+ save** — these are tarpit / objective-grabbing chaff. Do not commit to anti-tank shooting; they will evaporate to it.

---

## Source

- Wahapedia: https://wahapedia.ru/wh40k10ed/factions/tyranids/Hormagaunts
- Munitorum Field Manual v3.9 (Feb 2026), Codex: Tyranids section

<!-- Last verified: 2026-04-28 — source: Wahapedia https://wahapedia.ru/wh40k10ed/factions/tyranids/Hormagaunts / MFM v3.9 -->
